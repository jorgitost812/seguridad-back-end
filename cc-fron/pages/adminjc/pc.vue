    ...
    <div>
      ...
      <v-dialog v-model="dialog" max-width="600px">
        <v-card>
          ...
          <v-card-text>
            <v-text-field v-model="form.numero" label="N. Inventario" required></v-text-field>
            <v-text-field v-model="form.nombre" label="Nombre" required></v-text-field>
            <v-text-field v-model="form.ip" label="IP" required></v-text-field>
            <v-text-field v-model="form.setup" label="Pass Setup" type="password"></v-text-field>
            <v-text-field v-model="form.admin" label="Pass Admin" type="password"></v-text-field>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
      ...
      <v-dialog v-model="dialogEliminar" max-width="400px">
        <v-card>
          <v-card-title>¿Eliminar {{ pcAEliminar?.nombre }}?</v-card-title>
          <v-card-actions>
            <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      async cargarComputadoras() {
        ...
        this.cargando = true;
        try {
          const { data } = await this.$axios.get(`api/pcs/by_joven_club/${this.jcId}`);
          this.computadoras = data;
          console.log('Computadoras cargadas:', this.computadoras);
        } catch (error) {
      ...
      eliminarComputadora(item) {
        this.pcAEliminar = item;
        this.dialogEliminar = true;
      },
      async guardar() {
        if (!this.form.nombre || !this.form.numero || !this.form.ip) {
          this.$store.commit('alert/setAlert', {
            status: true,
            message: 'Nombre, inventario e IP son obligatorios',
            color: 'error'
          });
          return;
        }
      ...
      async confirmarEliminar() {
        try {
          await this.$axios.delete(`api/pcs/${this.pcAEliminar.id}`);
          this.$store.commit('alert/setAlert', {
            status: true,
          ...
          this.$store.commit('alert/setAlert', {
            status: true,
            message: error.response?.data?.message || 'Error al eliminar la computadora',
            color: 'error'
          });
        }
      }
