<template>
    <div>
      ...
      <v-data-table style="background-color: #FFF9C4" :headers="headers" :items="computadoras" :loading="cargando" class="elevation-1">
        <template v-slot:item.actions="{ item }"><v-icon small class="mr-2" @click="editarComputadora(item)">mdi-pencil</v-icon></template>
      </v-data-table>
  ...
  export default {
    layout: 'usuario',
    data: () => ({ cargando: false, computadoras: [], dialog: false, dialogTitulo: '', editandoId: null, form: { numero: '', nombre: '', ip: '', setup: '', admin: '' }, headers: [{ text: "Nombre", value: "nombre" }, { text: "Inventario", value: "numero" }, { text: "IP", value: "ip" }, { text: "Acciones", value: "actions", sortable: false }] }),
