<template>
 ...
 >
     ...
     <v-toolbar  color="primary" dark flat >
       ...
       <v-dialog v-model="dialog" >
         <template v-slot:activator="{ on, attrs }">
           ...
           <v-btn 
             v-if="user.rol.id === 1 || user.rol.id === 5"
             small class="ml-2" color="primary" fab v-bind="attrs" v-on="on">
           ...
           <v-card-text>
             <v-container>
               <v-row>
                 ...
                 <v-col cols="12" sm="6" md="4">
                   <v-text-field v-model="editedItem.apellidos" label="Apellidos" required></v-text-field>
                 </v-col>
                 <v-col cols="12" sm="6" md="4">
                   <v-text-field v-model="editedItem.email" label="Correo" type="email" required></v-text-field>
                 </v-col>
                 <v-col cols="12" sm="6" md="4">
                   <v-text-field v-model="editedItem.password" label="Contraseña" type="password"></v-text-field>
                 </v-col>
                 <v-col cols="12" sm="6" md="4">
                   <v-checkbox label="G. Municipal" v-model="editedItem.grupo_municipal"></v-checkbox>
                 </v-col>
   ...
   <template v-slot:item.actions="{ item }" >
     <template v-if="user.rol.id === 1 || user.rol.id === 5">
...
export default {
 ...
 computed: {
   canEditUser() {
     return this.user?.rol?.id === 1 || this.user?.rol?.id === 5;
   },
   formTitle() {
