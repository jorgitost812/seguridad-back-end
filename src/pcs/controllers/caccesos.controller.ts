...
export class cAccesosController {
  constructor(private caccesosService: cAccesosService) {}
  ...
  async create(@Body() createAccesoDto: CreateAccesoDto) {
    try {
      ...
      throw new BadRequestException(`Error al crear acceso: ${error.message}`);
    }
  }
  ...
  async getPDFByRange(
    ...
    //routes.get('/pdf', FileController.show);
    //console.log(json);
    const pdfStream = await this.caccesosService.generatePDFByRange(
      JSON.parse(json),
    );
    res
      .writeHead(200, {
